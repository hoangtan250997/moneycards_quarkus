...
public class PositionService {
    ...
    private void filterInactiveSkillAndTopic(PositionEntity positionEntity) {
        Predicate<RequiredSkillEntity> inactiveSkill = rs -> ACTIVE.equals(rs.getSkill().getStatus());
        Predicate<RequiredTopicEntity> inactiveTopic = rt -> ACTIVE.equals(rt.getTopic().getStatus());

        if (positionEntity.getRequiredSkillList() != null) {
            List<RequiredSkillEntity> filteredRequiredSkillList = positionEntity.getRequiredSkillList().stream()
                    .filter(inactiveSkill)
                    .collect(Collectors.toList());
            positionEntity.setRequiredSkillList(filteredRequiredSkillList);

            for (RequiredSkillEntity requiredSkillEntity : positionEntity.getRequiredSkillList()) {
                List<RequiredTopicEntity> filteredRequiredToicList = requiredSkillEntity.getRequiredTopicList().stream()
                        .filter(inactiveTopic)
                        .collect(Collectors.toList());
                requiredSkillEntity.setRequiredTopicList(filteredRequiredToicList);
            }
        }
    }
